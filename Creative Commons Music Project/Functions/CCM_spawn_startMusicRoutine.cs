...
namespace creativeCommonsMusicProject
{
    partial class CCM_core
    {
        ...
        internal static void CCM_spawn_startMusicRoutine(CCM_trackTypes_enum _trackType)
        {
            ...
            CCM_fnc_log.withTime.debug("CCM_spawn_startMusicRoutine: was called for scene " + _sceneName + " with track type: " + _trackType);
        ...
        {
            CCM_fnc_log.withTime.debug("CCM_spawn_startMusicRoutine: CCM_fnc_stopMusicRoutine: was called...");


            if (CCM_currentRoutine != null)
            {
                CCM_fnc_log.debug("CCM_spawn_startMusicRoutine: CCM_fnc_stopMusicRoutine: CCM_currentRoutine was NOT null, stopping...");
                CCM_Instance.StopCoroutine(CCM_currentRoutine);
                CCM_currentRoutine = null;
            }
            else
            {
                CCM_fnc_log.debug("CCM_spawn_startMusicRoutine: CCM_fnc_stopMusicRoutine: CCM_currentRoutine was null, nothing to stop");
            }
        ...
        {
            CCM_fnc_log.withTime.debug("CCM_spawn_startMusicRoutine: _fn_beginRoutine: was called for scene...");
            ...
            while (_sceneActive)
            {
                ...
                CCM_fnc_log.debug("CCM_spawn_startMusicRoutine: _fn_beginRoutine: New track has been selected for current scene: " + _sceneTrackFilename);
                ...
                while (_sleptTime < _sleepTime)
                {
                    ...
                    {
                        CCM_fnc_log.withTime.debug("CCM_spawn_startMusicRoutine: _fn_beginRoutine: Routine for scene: " + _sceneName + " is no longer the CCM_currentScene. Exiting loop...");
            ...
            CCM_fnc_log.withTime.debug("CCM_spawn_startMusicRoutine: _fn_beginRoutine: Routine for scene: " + _sceneName + " has exited its while loop.");   
        }
